...
- **Mevcut Sürüm:** `1.8.0`
- **Teknoloji:** Electron 33 + Node.js + Vanilla JavaScript, HTML, CSS (Framework kullanılmamaktadır). Paketlenme `electron-builder` 25 ile (NSIS x64) yapılmaktadır.
...
- **Dinamik Popover Sistemi:** 
  Tarih seçimi (`<input type="date">`) ve Notlar (`<textarea>`) için ortak bir popover altyapısı mevcuttur. Notlar popover'ı `resize: both` ile kullanıcı tarafından yeniden boyutlandırılabilir. Popover pozisyonları ekran kenarlarına taştığında (`positionPopover`) dinamik olarak ters yöne açılır (flip).
